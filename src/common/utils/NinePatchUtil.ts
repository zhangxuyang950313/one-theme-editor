...
export default class NinePatchUtil {
  ...
  static async encode9patchBatch(files: string[]): Promise<void> {
    console.time("耗时");
    console.log(`总计${files.length}个文件处理任务`);
    const cacheRecord = NinePatchUtil.readCache();
    console.log(`读取缓存${Object.keys(cacheRecord).length}个`);
    ...
    console.log(`去重后需要处理${md5List.length}个文件`);
    ...
    console.log(`需要开启${Math.ceil(md5List.length / taskCount)}个进程处理文件`);
    console.log(`每个 aapt 进程被分配${taskCount}个文件`);
